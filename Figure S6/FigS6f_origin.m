...
simulnum   = 50000;     % runs per parameter set
...
for s = 1:nSets
    ...
    % ----- runs -----
    for run = 1:simulnum
        if mod(run,50)==0, fprintf('  run %d/%d\n', run, simulnum); end
...
writematrix(mean_traj,'Fano_PNAS_Mean.csv')
writematrix(var_traj,'Fano_PNAS_Var.csv')

%% ================== Plots ========================================
mean_traj = readmatrix('Fano_PNAS_Mean.csv');
var_traj = readmatrix('Fano_PNAS_Var.csv');
